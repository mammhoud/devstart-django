...
```bash
docker compose -f docker-compose.local.yml run --rm django uv run manage.py makemessages --all --no-location
```
...
```bash
docker compose -f docker-compose.local.yml run --rm django uv run manage.py compilemessages
```
